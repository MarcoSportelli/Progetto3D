...
# Plot for each exercise (class) with aligned frames (all curves have the same length)
MAX_SAMPLES = 5

for class_name, all_angles in angles_per_class.items():
    if all_angles:
        plt.figure(figsize=(10, 5))
        for i, angles in enumerate(all_angles[:MAX_SAMPLES]):
            plt.plot(angles, alpha=0.5, label=f'Sample {i+1}')
        plt.xlabel("Frame")
        plt.ylabel("Angle (°)")
        plt.title(f"Angle trend - {PLOT_NAMES[class_name]}")
        plt.grid(True)
        plt.tight_layout()
    else:
        print(f"No correctly predicted samples for class '{class_name}'")
